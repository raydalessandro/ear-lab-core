...
**Stato:** fondamenta e Fase B implementate nel branch `feat/extraction-foundations`; il prossimo incremento è C1.
...
| B3 | `offline-queue` | La Famiglia → Moto-lollo | **Completato:** coda in-memory di `Operation`, retry e idempotenza non conoscono endpoint o payload di post; Moto-lollo resta candidato finché non ha un flusso reale. | Service worker/UI/hook React. |
...
| D3 | `notifications` | La Famiglia, 67_ENT | **Non promosso:** gli eventi sono già condivisi; solo La Famiglia ha un dispatcher multi-canale, 67_ENT gestisce inbox/subscription. Vedi `docs/NOTIFICATION_DISPATCH_PROMOTION_DECISION.md`. |
...
L’ordine di estrazione iniziale è concluso: C2 ha estratto il ledger puro comune ai serializzatori di Rocco-Zara e Scrivia; C1, C3, C4, D1, D2 e D3 sono stati **non promossi** fino a riuso reale. Il core contiene soltanto i contratti e le primitive con almeno due consumer o con un proof-of-concept di integrazione verificato; le decisioni documentate sono la coda di lavoro per future promozioni.
